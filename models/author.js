...
AuthorSchema.virtual('lifespan').get(function() {
  ...
  var bornDATE, deathDATE, lifetime_string;

  if (this.date_of_birth) {
    bornDATE = DateTime.fromJSDate(this.date_of_birth).toLocaleString(DateTime.DATE_MED);
    lifetime_string = bornDATE + ' - ';
  }

  if (this.date_of_death) {
    deathDATE = DateTime.fromJSDate(this.date_of_death).toLocaleString(DateTime.DATE_MED);
    return  lifetime_string + deathDATE;
  }
  else
    return lifetime_string;
});
